...
const FabricExplorer = () => {
  ...
  useEffect(() => {
    fetch('fabric_data_gain_lucky_2023_import (4).json')
  ...
  const suppliers = useMemo(() =>
    selectedBrand ? [...new Set(sampleData
      ...
      .map(d => d.Supplier))].sort() : [],
    [sampleData, selectedBrand]
  );

  const fabricTypes = useMemo(() =>
    selectedBrand && selectedSupplier ? [...new Set(sampleData
      .filter(d => d.Brand === selectedBrand && d.Supplier === selectedSupplier)
      .map(d => d.Normalized_fabric))].sort() : [],
    [sampleData, selectedBrand, selectedSupplier]
  );

  const materialData = useMemo(() => {
    if (!selectedBrand || !selectedSupplier || !selectedFabricType) return [];

    const filtered = sampleData.filter(d =>
      d.Brand === selectedBrand &&
      d.Supplier === selectedSupplier &&
      d.Normalized_fabric === selectedFabricType
    );

    const grouped = filtered.reduce((acc, curr) => {
      const material = curr.Fabric_Composition;
      if (!acc[material]) {
        acc[material] = 0;
      }
  ...
  }, [sampleData, selectedBrand, selectedSupplier, selectedFabricType]);
  ...
  const monthlyPriceData = useMemo(() => {
    ...
    const filtered = sampleData.filter(d =>
      ...
      d.Supplier === selectedSupplier &&
      d.Normalized_fabric === selectedFabricType &&
      d.Fabric_Composition === selectedMaterial
    );
    ...
    return data;
  }, [sampleData, selectedBrand, selectedSupplier, selectedFabricType, selectedMaterial]);
    ...
    <div className="w-full max-w-6xl mx-auto p-6 bg-white">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          {selectedBrand && selectedSupplier && selectedFabricType && (
            <button
        ...
        <p className="text-gray-600 mt-2">
          ...
          {currentView === 'supplier' && `Suppliers for ${selectedBrand}`}
          {currentView === 'fabric' && `Fabric types for ${selectedBrand} > ${selectedSupplier}`}
      ...
      {currentView === 'supplier' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {suppliers.map(supplier => {
            const totalQuantity = sampleData
              .filter(d => d.Brand === selectedBrand && d.Supplier === supplier)
              .reduce((acc, cur) => acc + cur.Quantity, 0);
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-semibold text-gray-800">{supplier}</h3>
      ...
      {currentView === 'material' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-2xl font-bold text-gray-800 mb-4 text-center">
            Material Composition by Quantity
          </h2>
          <p className="text-center text-gray-600 mb-6">
            Click on a section to view detailed analysis
          </p>
