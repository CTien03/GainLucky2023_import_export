...
const HierarchicalDataExplorer = () => {
  ...
  const [currentLevel, setCurrentLevel] = useState('brands');
  ...
  const COLORS = ['#8884d8', '#82ca9d', '#ffc658', '#ff7300', '#00ff00', '#ff00ff', '#00ffff', '#ff0000'];
    ...
    useEffect(() => {
      fetch('clothing_data_gain_lucky_2023_export_top20.json')
  ...
  const processData = useCallback(() => {
    ...
    if (selectedPath.buyer) {
      filteredData = filteredData.filter(item => item.buyer === selectedPath.buyer);
    }
  ...
  const handleSelection = (selectedItem) => {
    ...
    switch (currentLevel) {
      case 'brands':
        ...
        setCurrentLevel('buyer');
        break;
      case 'buyer':
        newPath.buyer = selectedItem;
        newBreadcrumb.push({ level: 'buyer', value: selectedItem });
  ...
  const handleBreadcrumbClick = (index) => {
    ...
    setSelectedPath(newPath);
    const levels = ['brands', 'buyer', 'fabric_type', 'product', 'percent_material', 'monthly_trends'];
    setCurrentLevel(levels[index + 1] || 'brands');
  };
    ...
    <div className="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow">
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        ...
        {selectedPath.product && (
          <button
            onClick={() => setCurrentLevel("percent_material")}
            className={`flex items-center gap-2 px-3 py-2 rounded-lg border-2 ${
      ...
      {/* Grid or Charts */}
      {["brands", "buyer", "fabric_type", "product"].includes(currentLevel) && (
